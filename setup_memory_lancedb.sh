...
REPO_PLUGIN_DIR="${PWD}/local-plugins/memory-lancedb"
...
if [[ ! -f "${REPO_PLUGIN_DIR}/index.ts" ]]; then
  echo "Could not find local memory-lancedb plugin copy at ${REPO_PLUGIN_DIR}" >&2
  exit 1
fi
...
cat <<EOF
OpenClaw benchmark state configured for memory-lancedb.
...
To switch back to the Pro backend:
